...
def adicionar_usuario():
    ...
    if not nome_user or not senha or not perfil:
        return jsonify({"message": "Todos os campos (nome_user, senha, perfil) são obrigatórios!"}), 400

    conn = get_db_connection()
    cursor = conn.cursor()
    query = "INSERT INTO Usuarios (nome_user, senha, perfil) VALUES (%s, %s, %s)"
    cursor.execute(query, (nome_user, senha, perfil))
    conn.commit()
    cursor.close()
    conn.close()
